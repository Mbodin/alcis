...
let rec load_options = function
    ...
    | action :: l ->
            let nb_needed = match Choices.get_nb_arg action with
                | Some n -> n
                | None -> Errors.error Position.global ["I’m really sorry, but I don’t know the option “" ^ action ^ "”."; "You can have the list of all available options with the option “-help”."]
            in
            let rec get_n_first = function
                | 0 -> fun l -> ([], l)
                | n -> function
                    | [] -> Errors.error Position.global ["I’m sorry but the option “" ^ action ^ "” needs " ^ (string_of_int nb_needed) ^ " argument, and only " ^ (string_of_int (nb_needed - n)) ^ " are given there."; "You can have some help with the option “-help”."]
